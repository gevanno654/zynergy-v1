  ...
  void _enableAllSchedules() async {
    ...
    if (_isSarapanEnabled) {
      _notificationService.rescheduleNotificationIfNeeded(
        1, 'Notifikasi Sarapan', 'Ingatlah untuk sarapan pagi!', DateTime(0, 0, 0, 7, 0),
      );
    }
    if (_isMakanSiangEnabled) {
      _notificationService.rescheduleNotificationIfNeeded(
        2, 'Notifikasi Makan Siang', 'Ingatlah untuk makan siang!', DateTime(0, 0, 0, 12, 0),
      );
    }
    if (_isMakanMalamEnabled) {
      _notificationService.rescheduleNotificationIfNeeded(
        3, 'Notifikasi Makan Malam', 'Ingatlah untuk makan malam!', DateTime(0, 0, 0, 18, 0),
      );
    }
...
<<<<<<< Updated upstream
  Future<void> _cancelNotification(String key) async {
    ...
    if (id != null) {
      log("Cancelling notification with ID: $id for key: $key");
      await _notificationService.cancelNotification(id);
    } else {
      log("No notification ID found for key: notification_$key. Skipping cancellation.");
    }
  }
